...
def get_grid(inputs):
  ...
  for line in inputs:
    ...
    if ord('a') in elevations:
      waypoints['starts'].append((len(grid), elevations.index(ord('a'))))
...
def process(inputs, debug=False):
  ...
  depths = [search(start, waypoints['end'], neighbors) for start in waypoints['starts']]
