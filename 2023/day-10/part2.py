    ...
    for row, line in enumerate(inputs):
        ...
        for col in range(len(line)):
            ...
            if prev_wall is not None:
                ...
                if is_prev_start and ((pipe.index(curr_pos) == 1) or pipe.index(curr_pos) == len(pipe) - 2):
                    continue

                is_curr_start = inputs[curr_pos[0]][curr_pos[1]] == "S"
                if is_curr_start and ((pipe.index(prev_wall) == 1) or pipe.index(prev_wall) == len(pipe) - 2):
                    continue
